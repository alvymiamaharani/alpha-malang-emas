...
const DEFAULT_WEIGHTS = [
  {
    ...
    weight: 0.25,
    criteria: [
      { code: "1.1", name: "Regulasi & Kedaulatan", weight: 0.10 },
      { code: "1.2", name: "Transparansi", weight: 0.10 },
      ...
      { code: "1.6", name: "Efektivitas & Efisiensi Pemerintahan", weight: 0.10 },
      { code: "1.7", name: "Kualitas Layanan Publik", weight: 0.10 },
      { code: "1.8", name: "Transformasi Digital / E-Gov & Smart City", weight: 0.10 },
      { code: "1.9", name: "Kepemimpinan (Leadership)", weight: 0.08 },
      { code: "1.10", name: "Keamanan Siber & Privasi Data", weight: 0.05 },
      { code: "1.11", name: "Keadilan & Pemerataan Layanan Publik", weight: 0.03 },
    ],
  },
  {
    code: "P2",
    ...
    criteria: [
      ...
      { code: "2.2", name: "Pendapatan Daerah (PAD) & Efisiensi Anggaran", weight: 0.06 },
      { code: "2.3", name: "Pendapatan Keluarga & Kesejahteraan", weight: 0.08 },
      ...
      { code: "2.10", name: "Kemiskinan & Ketimpangan (Gini, kesenjangan wilayah)", weight: 0.09 },
      { code: "2.11", name: "Produktivitas & Inovasi Ekonomi Lokal", weight: 0.07 },
      { code: "2.12", name: "Akses & Inklusi Keuangan", weight: 0.06 },
      { code: "2.13", name: "Ketahanan Ekonomi (pangan, harga pokok, shock)", weight: 0.07 },
  ...
  },
  {
    ...
    criteria: [
      ...
      { code: "3.2", name: "Kesehatan (AKI/AKB, Stunting, Harapan Hidup)", weight: 0.13 },
      { code: "3.3", name: "Hukum, HAM & Kesetaraan", weight: 0.1 },
      { code: "3.4", name: "Keagamaan, Toleransi & Kerukunan Sosial", weight: 0.08 },
      { code: "3.5", name: "Sosial-Budaya & Pariwisata", weight: 0.06 },
      { code: "3.6", name: "Keamanan & Ketertiban Sosial", weight: 0.08 },
      { code: "3.7", name: "Perlindungan Sosial & Inklusivitas (disabilitas & lansia)", weight: 0.12 },
  ...
  },
  {
    ...
    criteria: [
      ...
      { code: "4.5", name: "Pengelolaan Sampah & Circular Economy", weight: 0.12 },
      ...
      { code: "4.9", name: "Pengelolaan SDA & Keanekaragaman Hayati", weight: 0.11 },
    ],
  },
  {
    code: "P5",
    ...
    weight: 0.15,
    criteria: [
      { code: "5.1", name: "Keberlanjutan Ekonomi melalui Inovasi (Green Economy)", weight: 0.12 },
      { code: "5.2", name: "Investasi R&D & Pendidikan", weight: 0.12 },
      { code: "5.3", name: "Keterlibatan Pemangku Kepentingan", weight: 0.10 },
      { code: "5.4", name: "Dampak Sosial & Lingkungan dari Inovasi", weight: 0.12 },
      { code: "5.5", name: "Efektivitas Teknologi", weight: 0.10 },
      { code: "5.6", name: "Kebijakan & Regulasi Pro-Inovasi (HKI, insentif)", weight: 0.10 },
      { code: "5.7", name: "Internasionalisasi Inovasi (GII, paten, publikasi)", weight: 0.08 },
      { code: "5.8", name: "Inovasi Digital & Smart Society", weight: 0.10 },
      { code: "5.9", name: "Ekosistem Startup & Bisnis Inovatif", weight: 0.08 },
      { code: "5.10", name: "Evaluasi Dampak Inovasi (adopsi, kepuasan, keberlanjutan)", weight: 0.08 },
    ],
  },
];
  ...
  weights.forEach((P) => {
    ...
    const SP = criteria.reduce((sum, C) => {
      ...
      if (items.length > 0) {
        ...
        SK = items.reduce((acc, it) => acc + ((totalW ? it.weight / totalW : 1 / items.length) * clampScore(it.score)), 0);
      } else {
        const fallback = typeof entries[critKey] === "number" ? clampScore(entries[critKey]) : 0;
        SK = fallback;
      }
...
export default function EMASMobileApp() {
  ...
  const result = useMemo(() => computeScores(weights, scores, indicators), [weights, scores, indicators]);
  ...
  async function saveToSupabase() {
    ...
    try {
      ...
      const { error } = await supabase.from("assessments").insert({ user_email: email, title, payload });
  ...
  function exportJson() {
    const blob = new Blob(
      [
        JSON.stringify(
          { title, weights, scores, indicators, result, exported_at: new Date().toISOString() },
          null,
          2,
        ),
      ],
  ...
  function addIndicator(critKey) {
    ...
    const next = { id: uid(), name: `Indikator ${list.length + 1}`, weight: 1, score: 3 };
  ...
  function removeIndicator(critKey, id) {
    const list = indicators[critKey] || [];
    setIndicators({ ...indicators, [critKey]: list.filter((it) => it.id !== id) });
  }
    ...
    <div className="min-h-dvh w-full bg-background text-foreground pb-24">
            ...
            <SheetContent side="left" className="w-[85vw] sm:w-80">
              ...
              <div className="mt-4 space-y-4 text-sm mx-4">
                <div className="space-y-2">
                  <Label htmlFor="email">Email (untuk penyimpanan Supabase)</Label>
                  <Input id="email" placeholder="nama@contoh.com" value={email} onChange={(e) => setEmail(e.target.value)} />
                </div>
                <div className="space-y-2">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button onClick={saveToSupabase} disabled={saving} className="gap-2" variant="default">
                      <Save className="h-4 w-4" /> Simpan
                    </Button>
                    <Button onClick={exportJson} variant="secondary" className="gap-2">
                      <Download className="h-4 w-4" /> Ekspor JSON
                    </Button>
                    <Button onClick={resetScores} variant="outline" className="col-span-2">
                      Reset Skor
                    </Button>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Button size="sm" variant="outline" onClick={loadHistory} className="gap-2">
                  ...
                  <ScrollArea className="h-48 rounded-md border p-2">
                    {loadingHistory ? (
                      ...
                      <div className="space-y-2">
                        {history.map((row) => (
                          <div key={row.id} className="flex items-center justify-between rounded-lg border p-2">
                            <div className="text-xs">
                              <div className="font-medium">{row.title || "Tanpa Judul"}</div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Button size="icon" variant="ghost" onClick={() => deleteEntry(row.id)} aria-label="Hapus">
                ...
                <div className="text-xs text-muted-foreground">
                  <p className="mb-1">Formula: tertimbang (Prinsip × Kriteria × (opsional) Indikator)</p>
        ...
        <Card className="shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">Mobile · Weighted</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="flex items-center gap-4 py-2">
              ...
              <div className="space-y-1">
                <div className="text-sm text-muted-foreground">Skor Akhir (1–5)</div>
                <div className="text-2xl font-bold">{result.SA.toFixed(2)}</div>
                <div className="text-sm text-muted-foreground">Karat (24K)</div>
                <div className="text-xl font-semibold">{result.karat.toFixed(2)}K</div>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <TabsContent key={P.code} value={P.code} className="space-y-3 pt-2">
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="text-xs text-muted-foreground">Bobot prinsip: {(P.weight * 100).toFixed(0)}%</div>
                  <div className="grid grid-cols-1 gap-3">
                    {P.criteria.map((C) => {
                      const key = `${P.code}|${C.code}`;
                      const hasIndicators = (indicators[key]?.length || 0) > 0;
                      const val = typeof scores[key] === "number" ? scores[key] : 3; // default mid
                              ...
                              <Dialog>
                                <DialogTrigger asChild>
                                  <Button size="sm" variant="outline" className="gap-2">
                                    <ListOrdered className="h-4 w-4" /> Indikator
                                  </Button>
                                </DialogTrigger>
                                <DialogContent className="max-w-md">
                                  ...
                                  <div className="space-y-3">
                                    <div className="text-xs text-muted-foreground">
                                      Isi indikator bila ingin penilaian lebih rinci. Jika kosong, nilai kriteria (slider) dipakai.
                                    </div>
                                    <div className="space-y-2">
                                      {(indicators[key] || []).map((it) => (
                                        <div key={it.id} className="rounded-lg border p-2">
                                          <div className="flex items-center gap-2">
                                            <Input
                                              value={it.name}
                                              onChange={(e) => updateIndicator(key, it.id, { name: e.target.value })}
                                              className="flex-1"
                                            />
                                            <Button
                                              size="icon"
                                              variant="ghost"
                                              onClick={() => removeIndicator(key, it.id)}
                                              aria-label="hapus"
                                            >
                                              <Trash2 className="h-4 w-4" />
                                            </Button>
                                            ...
                                            <div>
                                              ...
                                              <div className="flex items-center gap-2">
                                                <Slider
                                                  ...
                                                  step={0.5}
                                                  value={[it.score]}
                                                  onValueChange={(v) => updateIndicator(key, it.id, { score: clampScore(v[0]) })}
                                                  className="flex-1"
                                                />
                                                <Input
                                                  type="number"
                                                  ...
                                                  onChange={(e) =>
                                                    updateIndicator(key, it.id, {
                                                      score: clampScore(parseFloat(e.target.value || "0")),
                                                    })
                                                  }
                                                />
                                              </div>
                                    ...
                                    <Button onClick={() => addIndicator(key)} variant="secondary">
                                      Tambah Indikator
                                    </Button>
                                  </div>
                                  <DialogFooter>
                                    <div className="text-xs text-muted-foreground">Rumus SK: Σ( wInd_norm × SI )</div>
                                  </DialogFooter>
                                </DialogContent>
                              </Dialog>
                            </div>
                          ...
                          <div className="mt-2 flex items-center gap-3">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setScores({ ...scores, [key]: clampScore(isNaN(n) ? 3 : n) });
                              }}
                              disabled={hasIndicators}
                            />
                          </div>
                          <div className="mt-1 flex items-center justify-between text-[11px] text-muted-foreground">
                            <span>Bobot kriteria: {Math.round(C.weight * 100)}%</span>
                            <span>{hasIndicators ? "Menggunakan skor indikator" : "Menggunakan skor kriteria"}</span>
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <div className="pt-1 text-xs text-muted-foreground">
                    Skor Prinsip (SP): {result.perPrinciple[P.code]?.SP?.toFixed(2)} • Karat:{" "}
      ...
      <nav className="fixed bottom-0 left-0 right-0 border-t bg-background/90 backdrop-blur supports-[backdrop-filter]:bg-background/70 z-40">
        <div className="mx-auto max-w-md px-3 py-2 grid grid-cols-3 gap-2">
          ...
          <Button variant="default" className="gap-2" onClick={saveToSupabase} disabled={saving}>
            <Save className="h-4 w-4" /> Simpan
          </Button>
